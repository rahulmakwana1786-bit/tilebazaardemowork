...
export default function CheckoutPage() {
  ...
  const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // If postcode changes, try to fetch immediately
    if (name === "postcode") {
      ...
      if (isValid) {
        try {
          ...
          if (response.data && response.data.deliveryCharge !== undefined) {
  ...
  const handlePostcodeBlur = async () => {
    ...
    setErrors((prev) => ({
      ...prev,
      postcode: isValid ? "" : "Wrong postcode",
    }));
              ...
              {cartItems.length === 0 ? (
                ...
                <>
                        ...
                        <div key={item.id} className="flex gap-4">
                          ...
                          <div className="flex flex-col justify-center">
                            ...
                            <p className="text-[11px] text-gray-500">
                              £{getProductPrice(product).toFixed(2)} {getUnitString(product)}
                            </p>
                          </div>
                  ...
                  <div className="border-t border-gray-200 pt-6 space-y-4 mb-8">
                    {cartItems.some(item => !(item.product as any)?.category?.toUpperCase().includes('ACCESS') && !(item.product as any)?.category?.toUpperCase().includes('ADHESIVE')) && (
                      <div className="grid grid-cols-3 gap-2 mb-6 bg-gray-50 p-3 rounded-sm border border-gray-100">
                        <div>
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-500">£{deliveryCharge.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                  ...
                  <div className="border-t border-[#4a2c2a] pt-6 mb-8">
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-2xl font-serif text-[#4a2c2a]">£{(totalPrice * 1.20 + deliveryCharge).toFixed(2)}</span>
                    </div>
                  </div>
